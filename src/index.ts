...
import { CatalogChangeEvent, Events, IFormErrors, IShopItem, IPaymentType } from './types';
...
events.on<CatalogChangeEvent>(Events.LOAD_ITEMS, () => {
	...
	page.gallery = appData.catalog.map((item) => {
		const card = new Card('card', cloneTemplate(cardCatalogTemplate), events, {
			onClick: () => events.emit(Events.OPEN_ITEM, item),
		});
		return card.render({
...
events.on(Events.OPEN_ITEM, (item: IShopItem) => {
	// Отображаем результат
	const card = new Card('card', cloneTemplate(cardPreviewTemplate), events, {
		onClick: () => {
			if (appData.isShopItemInBasket(item)) {
				item.removeFromBasket();
			} else {
				item.placeInBasket();
			}
			events.emit(Events.OPEN_ITEM, item);
		},
	});
...
// Любые изменения в любом из предметов
events.on(Events.CHANGE_ITEM_IN_BASKET, () => {
